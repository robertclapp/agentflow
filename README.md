...
That wrapper runs the local bash/Kimi toolchain check, the external custom-pipeline `doctor`, `inspect`, `check-local`, and `run` paths for `bootstrap: kimi`, explicit `shell_init: kimi`, and explicit `target.shell` Kimi wrappers, plus the bundled `agentflow check-local` flow. It now also prints whether `~/.bash_profile`, `~/.bash_login`, or `~/.profile` is supplying the bash login startup path before it verifies `kimi`, `codex`, and `claude`, and it reports whether Codex auth is coming from `OPENAI_API_KEY`, Codex CLI login, or both in that shared Kimi-backed shell.
...
On hosts that rely on Bash falling back to `~/.profile`, a healthy setup can still print `~/.bash_profile: missing` and `~/.bash_login: missing`; the key line is the resolved `bash login startup: ...` chain. When that chain is broken or shadowed, the maintainer check now follows it with either `bash login bridge: not needed` or the explicit target, source, reason, and snippet to paste into the active login file.
Those maintainer shell probes now fail fast too: the default budget is 15 seconds per wrapped command, `AGENTFLOW_LOCAL_VERIFY_TIMEOUT_SECONDS` overrides that maintainer-specific budget, and when that variable is unset the scripts fall back to `AGENTFLOW_DOCTOR_TIMEOUT_SECONDS` so Doctor and maintainer checks can share the same slower-shell override.
